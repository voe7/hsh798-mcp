...
let currentRandom: number = 0;

async function getCodeImg(): Promise<string> {
    currentRandom = Math.random();
    const timestamp = Date.now();
    const url = `${basicURL}/api/v1/captcha/?s=${currentRandom}&r=${timestamp}`;
...
// Export currentRandom dynamically for backward compatibility with login.ts
export { currentRandom as random };